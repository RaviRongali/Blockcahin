...
jsonfile = "outputpeer_adversary_interarrival:"+str(inter_arrival_time)+"_floodpercent:"+str(flood_percent)+"_" + my_ip + ":" + str(my_peer_server_port) + ".json"
queue =[]
init_queue=[]
...
def flooding_attack():
    totalconnections=len(all_peer_servers)+len(all_peer_clients)
    flooded=max(1,flood_percent)
    totallist=all_peer_servers+all_peer_clients
    floodednodes=random.sample(totallist, k=flooded)
    while True:    
        if totalconnections!=len(all_peer_servers)+len(all_peer_clients):
            currenttotal=len(all_peer_servers)+len(all_peer_clients)
            to_be_flood=max(1,flood_percent)
            if to_be_flood>flooded:
                remain=to_be_flood-flooded
                toconnections=all_peer_servers+all_peer_clients
                noflooded=toconnections-floodednodes
                toflood=random.sample(noflooded, k=remain)
                for node in toflood:
                    floodednodes.append(node)
        message = "00" + str("null")+ str("null")+str("null")+"***"
        for peer in floodednodes:
            try:
                s = peer[0]
                s.send(str.encode(message))
                # print("server ",peer[1],message)
            except:
                print(" unable to send flodding message to server: ", peer[1][1])
        
        # print("broadcasting block 648")
        t = inter_arrival_time * 0.1
        time.sleep(t)
